// Simple convolution benchmark (sequential version, using a mapTo over a TypedObject array of indices for the inner loop)
// 2014-03-21 / lhansen@mozilla.com
//
// For testing, run like this:
//
//  js cat-convolve-mapTo-TypedObject-Array.js | ./unhex > out.pgm
//
// For benchmarking, set 'benchmark' to true and run like this:
//
//  js cat-convolve-mapTo-TypedObject-Array.js
...
var indices = IX.build(width-2, x => x+1) // [1,2,...,width-2]
var result = [];
for ( var i=0 ; i < height-2 ; i++ )
    result.push(new IX(width-2));
var r = time(
    function () {
	var r;
	for ( var i=0 ; i < iterations ; i++ ) {
	    r = null;
	    r = edgeDetect1(bytes, indices, loc, height, width, result);
	}
	return r;
    });
...
function edgeDetect1(input, indices, loc, height, width, result) {

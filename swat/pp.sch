...
; [From the Larceny sources, May 1, 2018.  Since modified.]
;
...
(let ()
  ...
  (define (%generic-write obj display? unicode? width output lvl0 len0)
    ...
    (define (pp obj col lvl)
      ...
      (define max-call-head-width 120)
  ...
  ;; opt is either empty, a port, or kwd/value pairs
  ;; 'port    port
  ;; 'display bool

  (define (pretty obj . opt)
    (define port-opt #f)
    (define display-opt #f)

    (cond ((= (length opt) 1)
           (set! port-opt (car opt)))
          ((even? (length opt))
           (do ((opt opt (cddr opt)))
               ((null? opt))
             (case (car opt)
               ((port) (set! port-opt (cadr opt)))
               ((display) (set! display-opt (cadr opt)))
               (else (error "Bad option" (car opt))))))
          (else
           (error "Bad options to pp" opt)))

    (let* ((port (or port-opt (current-output-port)))

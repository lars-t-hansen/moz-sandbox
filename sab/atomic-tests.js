...
// Something screwy here: why do these methods need to be called with
// op.call(Atomics, ...)  when they really should be callable as
// functions?
//
// cf Math.
//
// This could be related to my use in the implementation of CallNonGenericMethod!
...
function testTypeBinop(a, op) {
    ...
    op.call(Atomics, a, 0, 0.7); // doubles are allowed, this should not throw

    var thrown = false;
    try {
	op.call(Atomics, a, 0, "0");
    }
    catch (e) {
	thrown = true;
	assertEq(e instanceof TypeError, true);

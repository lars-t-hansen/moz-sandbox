...
class VEnv {
    private env:  VEnvMap;
    private outer:VEnv;
    constructor(env:VEnvMap, outer?:VEnv) {
	this.env = env;
	this.outer = outer;
    }
...
interface FDesc {
    arity:number;
    code: (env:VEnv, actuals:number[]) => number;
}
...
var venv:VEnv = new VEnv({ "PI": Math.PI });
var fenv:FEnv = { "sin": { arity: 1, code: function (env, args:number[]) { return Math.sin(args[0]) } },
		  "cos": { arity: 1, code: function (env, args:number[]) { return Math.cos(args[0]) } },
		  "pow": { arity: 2, code: function (env, args:number[]) { return Math.pow(args[0], args[1]) } } };

function main() {
    var s:string;
    while (s = readline()) {
	try {
	    print(evaluate(venv, parse(lex(s))));
	}
	catch (e) {
	    print(e);
	}
...
function evaluate(env:VEnv, t: Tree) : number {
    switch (t.tag) {
      case Ast.Defn: {
	  var d:Defn = <Defn> t;
      ...
      case Ast.Assign: {
	  var a:Assign = <Assign> t;
	  var v:number = evaluate(env, a.val);
	  env.update(a.name, v);
	  return v;
      }
      case Ast.Call: {
	  ...
	  var vs:number[] = c.args.map(function (x) { return evaluate(env, x); });
	  return f.code(env, vs);
      }
      case Ast.Unop: {
	  var u:Unop = <Unop> t;
	  var o:number = evaluate(env, u.opd);
	  switch (u.opr) {
      ...
      case Ast.Binop: {
	  var b:Binop = <Binop> t;
	  var l:number = evaluate(env, b.lhs);
	  var r:number = evaluate(env, b.rhs);
	  switch (b.opr) {
      ...
      case Ast.VarRef: {
	  return env.lookup((<VarRef> t).name);
      }
      case Ast.NumLit: {
...
function makeFunction(formals:string[], body:Tree): any {
    return function (env:VEnv, actuals:number[]) {
	var newrib:VEnvMap = {};
	for ( var i=0 ; i < formals.length ; i++ )
	    newrib[formals[i]] = actuals[i];
	return evaluate(new VEnv(newrib, env), body)
    }
}
...
function parse(ts:Token[]): Tree {
	...
	var name:string = (<Ident>ts[0]).name;
	ts.shift();
	...
	while ((t = peek(ts)) != Tok.EOI && t != Tok.RightParen) {
	    ...
	    var param:string = (<Ident>ts[0]).name;
	    ts.shift();
	    formals.push(param);
	}
	match(ts, Tok.RightParen);
	var body:Tree = parse0(ts);
...
function parse0(ts:Token[]): Tree {
    var e:Tree = parse1(ts);
    for (;;) {
	var t:Tok;
	if ((t = peek(ts)) != Tok.Plus && t != Tok.Minus)
	    break;
	ts.shift();
	var e2:Tree = parse1(ts);
...
function parse1(ts:Token[]): Tree {
    var e:Tree = parse2(ts);
    for (;;) {
	var t:Tok;
	if ((t = peek(ts)) != Tok.Times && t != Tok.Divide)
	    break;
	ts.shift();
	var e2:Tree = parse2(ts);
	e = new Binop(translate(t), e, e2);
    }
    return e;
}
...
function parse2(ts:Token[]): Tree {
    switch (peek(ts)) {
      ...
      case Tok.Number: {
	  var value:number = (<Num> ts[0]).value;
	  ts.shift();

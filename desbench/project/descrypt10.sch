...
;  - initial/final permutations open-coded as table lookups in small table.
;
; Lars Thomas Hansen.
; December 6, 1996
;
...
(define (des-process-v text keys s-boxes ip-m ip-inverse-m)
  ...
  (define (permute-vec permutation s shift-set)
    (let ((v (make-bitvector 64 0)))
      (do ((i 0 (+ i 1))
	   (j 8 (+ j 8)))
	  ((= i 8) v)
	(let* ((n (unbits-vec (and-vec (shr-vec s (- 64 j)) mask255)))
	       (w (vector-ref permutation n)))
	  (or-vec! v (shift w (vector-ref shift-set i)))))))

  (define (ip-permute v)
    (permute-vec ip-m v '#(0 -1 -2 -3 -4 -5 -6 -7)))

  (define (ip-unpermute v)
    (permute-vec ip-inverse-m v '#(0 2 4 6 -1 1 3 5)))

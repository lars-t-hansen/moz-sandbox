...
//
// The language is JavaScript for the SpiderMonkey shell.
...
function main() {
    setStage();
    var then = Date.now();
    var rows = trace(0, height);
    for ( var h=0 ; h < height ; h++ )
	for ( var w=0 ; w < width ; w++ )
	    bits.set(h, w, rows[h][w]);
    var now = Date.now();
    if (benchmark)
...
function trace(hmin, hlim) {
    if (antialias)
	return traceWithAntialias(hmin, hlim);
    else
	return traceWithoutAntialias(hmin, hlim);
}

function traceWithoutAntialias(hmin, hlim) {
    var rows = 
	Array.buildPar(hlim-hmin,
		       function (_h) {
			   const h=_h+hmin;
			   var row = []; // PJS BUG: new Array(width) is not supported
			   for ( var w=0 ; w < width ; w++ ) {
			       const u = left + (right - left)*(w + 0.5)/width;
			       const v = bottom + (top - bottom)*(h + 0.5)/height;
			       const ray = DL3(u, v, -eye.z);
			       row[w] = raycolor(eye, ray, 0, SENTINEL, reflection_depth);
			   }
			   return row;
		       });
    return rows;
...
function traceWithAntialias(hmin, hlim) {
	...
	Array.buildPar(hlim-hmin,
			   ...
			   const h=_h+hmin;
			   var row = [];
			   var k = h % 32; // Change in behavior from the serial version
			   for ( var w=0 ; w < width ; w++ ) {
